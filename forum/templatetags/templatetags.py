...
def pluralize_0(value, arg="s"):
    """
    Return a plural suffix if the value is not 1, '1', or an object of
    length 1. By default, use 's' as the suffix:

    * If value is 0, vote{{ value|pluralize }} display "vote".
    * If value is 1, vote{{ value|pluralize }} display "vote".
    * If value is 2, vote{{ value|pluralize }} display "votes".

    If an argument is provided, use that string instead:

    * If value is 0, class{{ value|pluralize:"es" }} display "class".
    ...
    * If value is 0, cand{{ value|pluralize:"y,ies" }} display "candy".
    ...
    * If value is 2, cand{{ value|pluralize:"y,ies" }} display "candies".
    ...
    try:
        return singular_suffix if float(value) == 1 or float(value) == 0 else plural_suffix
    except ValueError:  # Invalid string that's not a number.
        pass
    except TypeError:  # Value isn't a string or a number; maybe it's a list?
        try:
            return singular_suffix if len(value) == 1 or len(value) == 0 else plural_suffix

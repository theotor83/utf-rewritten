...
def post_redirect(request, postid):
    ...
    public_params = ['date','style','q'] # Any other parameter will be ignored. Add other public parameters as needed. 
    query_params = {}
    ...
    if page_redirect == 1:
        if query_params:
            return redirect(f"{reverse('topic-details', args=[topic.id, topic.slug])}?{urlencode(query_params)}#p{postid}")
        else:
            return redirect(f"{reverse('topic-details', args=[topic.id, topic.slug])}#p{postid}")
    else:
        if query_params:
            return redirect(f"{reverse('topic-details', args=[topic.id, topic.slug])}?{urlencode(query_params)}&page={page_redirect}#p{postid}")
        else:

...
from archive.models import ArchiveProfile

def delete_all_archive_profiles():
    """
    Deletes all instances of ArchiveProfile from the database
    and resets the primary key sequence for SQLite.
    """
    db_alias = 'archive'
    table_name = ArchiveProfile._meta.db_table

    # First, delete all the objects using the ORM.
    print("Deleting all ArchiveProfile instances...")
    count, _ = ArchiveProfile.objects.using(db_alias).all().delete()
    print(f"{count} ArchiveProfile instances have been deleted.")

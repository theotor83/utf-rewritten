...
from archive.models import ArchivePost

def delete_all_archive_posts():
    """
    Deletes all instances of ArchivePost from the database
    and resets the primary key sequence for SQLite.
    """
    db_alias = 'archive'
    table_name = ArchivePost._meta.db_table

    # First, delete all the objects using the ORM.
    print("Deleting all ArchivePost instances...")
    count, _ = ArchivePost.objects.using(db_alias).all().delete()
    print(f"{count} ArchivePost instances have been deleted.")

...
from archive.models import ArchivePoll, ArchivePollOption, ArchivePollOptionVoters

def delete_all_polls():
    """
    Deletes all instances of ArchivePoll from the database
    and resets the primary key sequence.
    """
    db_alias = 'archive'
    connection = connections[db_alias]
    table_name = ArchivePoll._meta.db_table

    print("Deleting all ArchivePoll instances...")
    try:
...
def delete_all_poll_options():
    """
    Deletes all instances of ArchivePollOption from the database
    and resets the primary key sequence.
    """
    db_alias = 'archive'
    connection = connections[db_alias]
    table_name = ArchivePollOption._meta.db_table

    print("Deleting all ArchivePollOption instances...")
    try:
        with connection.cursor() as cursor:
            # Use raw SQL to delete all rows, which is faster and avoids model validation issues
            cursor.execute(f'DELETE FROM "{table_name}";')
...
def delete_all_poll_options_voters():
    """
    Deletes all instances of ArchivePollOptionVoters from the database
    and resets the primary key sequence.
    """
    db_alias = 'archive'
    connection = connections[db_alias]
    table_name = ArchivePollOptionVoters._meta.db_table

    print("Deleting all ArchivePollOptionVoters instances...")
    try:
        with connection.cursor() as cursor:
            # Use raw SQL to delete all rows, which is faster and avoids model validation issues
            cursor.execute(f'DELETE FROM "{table_name}";')

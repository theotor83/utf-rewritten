...
from archive.models import ArchiveCategory

def delete_all_archive_categories():
    """
    Deletes all instances of ArchiveCategory from the database
    and resets the primary key sequence for SQLite.
    """
    db_alias = 'archive'
    table_name = ArchiveCategory._meta.db_table

    # First, delete all the objects using the ORM.
    print("Deleting all ArchiveCategory instances...")
    count, _ = ArchiveCategory.objects.using(db_alias).all().delete()
    print(f"{count} ArchiveCategory instances have been deleted.")

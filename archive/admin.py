from django.contrib import admin
from . import models
from django.utils.html import format_html
from django.urls import reverse
from django.template.defaultfilters import truncatechars
from django.contrib.auth.models import User

# Inlines

class ArchivePostInline(admin.TabularInline):
    model = models.ArchivePost
    fields = ('author', 'text', 'created_time')
    readonly_fields = ('author', 'created_time')
    extra = 0
    ordering = ('created_time',)
    show_change_link = True

class ArchiveTopicInline(admin.TabularInline):
    model = models.ArchiveTopic
    fields = ('title', 'author', 'created_time', 'is_sub_forum', 'is_locked')
    readonly_fields = ('author', 'created_time')
    extra = 0
    show_change_link = True

class ArchivePollOptionInline(admin.TabularInline):
    model = models.ArchivePollOption
    fields = ('text', 'vote_count_display')
    readonly_fields = ('vote_count_display',)
    extra = 1

    def vote_count_display(self, obj):
        return obj.get_vote_count
    vote_count_display.short_description = "Votes"

class ArchivePollOptionVotersInline(admin.TabularInline):
    model = models.ArchivePollOption.voters.through
    verbose_name = "Voter"
    verbose_name_plural = "Voters"
    extra = 1

class ArchiveProfileInline(admin.TabularInline): # For ArchiveForumGroup
    model = models.ArchiveProfile.groups.through
    verbose_name = "Member"
    verbose_name_plural = "Members"
    extra = 0

    #def profile_user(self, instance):
    #    return instance.profile.user.username
    #profile_user.short_description = 'User'


# ModelAdmins

@admin.register(models.ArchiveProfile)
class ArchiveProfileAdmin(admin.ModelAdmin):
    list_display = ('get_user_info', 'get_top_group_display', 'messages_count', 'birthdate', 'last_login')
    search_fields = ('user_id',)
    list_filter = ('groups', 'gender', 'type')
    #inlines = [ArchivePostInline]
    readonly_fields = ('last_login', 'display_user_posts', 'user')
    fieldsets = (
        (None, {
            'fields': ('user', 'profile_picture', 'groups', 'messages_count', 'last_login')
        }),
        ('Personal Info', {
            'fields': ('birthdate', 'gender', 'type', 'zodiac_sign', 'desc', 'localisation', 'loisirs')
        }),
        ('Contact & Preferences', {
            'fields': ('website', 'skype', 'email_is_public')
        }),
        ('Forum Specific', {
            'fields': ('favorite_games', 'signature', 'display_user_posts')
        }),
        ('Other', {
            'fields': ('upload_size',)
        }),
    )
    filter_horizontal = ('groups',)

    def get_user_info(self, obj):
        try:
            user = User.objects.using('default').get(pk=obj.user_id)
            url = reverse('admin:auth_user_change', args=[user.pk])
            return format_html('<a href="{}">{}</a>', url, user.username)
        except User.DoesNotExist:
            return f"User ID: {obj.user_id} (Not found)"
    get_user_info.short_description = "User"

    def get_top_group_display(self, obj):
        top_group = obj.get_top_group
        if top_group:
            return format_html('<span style="color: {};">{}</span>', top_group.color, top_group.name)
        return "N/A"
    get_top_group_display.short_description = "Top Group"
    get_top_group_display.admin_order_field = 'groups__priority' # Allows sorting

    def display_user_posts(self, obj):
        if obj.user:
            posts = models.ArchivePost.objects.filter(author=obj.user).order_by('-created_time')[:10] # Display recent 10 posts
            if not posts:
                return "No posts by this user."
            
            html = "<strong>Recent Posts (up to 10):</strong><ul>"
            for post in posts:
                post_admin_url = reverse('admin:archive_archivepost_change', args=[post.pk])
                topic_title = post.topic.title if post.topic else "N/A"
                html += f'<li><a href="{post_admin_url}">{topic_title} ({post.created_time.strftime("%Y-%m-%d %H:%M")})</a>: {truncatechars(post.text, 75)}</li>'
            html += "</ul>"
            
            all_posts_url = reverse('admin:archive_archivepost_changelist') + f'?author__id__exact={obj.user.pk}'
            html += f'<p><a href="{all_posts_url}">View all posts by this user ({obj.messages_count})</a></p>'
            return format_html(html)
        return "N/A"
    display_user_posts.short_description = "User Posts"

@admin.register(models.ArchiveForumGroup)
class ArchiveForumGroupAdmin(admin.ModelAdmin):
    list_display = ('name', 'priority', 'is_staff_group', 'is_messages_group', 'minimum_messages', 'member_count')
    search_fields = ('name',)
    list_filter = ('is_staff_group', 'is_messages_group')
    # inlines = [ArchiveProfileInline] # This can be very heavy if there are many users.
                               # I already have filter_horizontal on ArchiveProfileAdmin for 'groups'
    ordering = ('-priority',)

    def member_count(self, obj):
        return obj.archive_users.count()
    member_count.short_description = "Members"

@admin.register(models.ArchivePost)
class ArchivePostAdmin(admin.ModelAdmin):
    list_display = ('__str__', 'author_link', 'topic_link', 'created_time', 'update_count')
    search_fields = ('text', 'author_id', 'topic__title')
    list_filter = ('created_time',)
    readonly_fields = ('created_time', 'updated_time', 'author')
    date_hierarchy = 'created_time'
    ordering = ('-created_time',)

    def author_link(self, obj):
        if obj.author:
            return format_html('<a href="{}">{}</a>', 
                               f'/admin/auth/user/{obj.author.pk}/change/', 
                               obj.author.username)
        return "N/A"
    author_link.short_description = "Author"

    def topic_link(self, obj):
        if obj.topic:
            return format_html('<a href="{}">{}</a>', 
                               f'/admin/archive/archivestopic/{obj.topic.pk}/change/', 
                               obj.topic.title)
        return "N/A"
    topic_link.short_description = "Topic"


@admin.register(models.ArchiveTopic)
class ArchiveTopicAdmin(admin.ModelAdmin):
    list_display = ('title', 'author_link', 'category_link', 'parent_link', 'created_time', 'last_message_time', 'total_replies', 'total_views', 'is_sub_forum', 'is_locked', 'has_poll_display')
    search_fields = ('title', 'description', 'author_id')
    list_filter = ('is_sub_forum', 'is_locked', 'is_pinned', 'is_announcement', 'category', 'created_time')
    readonly_fields = ('created_time', 'last_message_time', 'author')
    inlines = [ArchivePostInline] # Shows posts directly in the topic admin page
    ordering = ('-last_message_time',)
    fieldsets = (
        (None, {
            'fields': ('title', 'author', 'slug', 'description', 'icon')
        }),
        ('Hierarchy & Type', {
            'fields': ('category', 'parent', 'is_sub_forum')
        }),
        ('Status & Stats', {
            'fields': ('is_locked', 'is_pinned', 'is_announcement', 'is_index_topic', 'total_views', 'total_replies')
        }),
        ('Timestamps', {
            'fields': ('created_time', 'last_message_time')
        }),
    )

    def author_link(self, obj):
        if obj.author:
            return format_html('<a href="{}">{}</a>', 
                               f'/admin/auth/user/{obj.author.pk}/change/', 
                               obj.author.username)
        return "N/A"
    author_link.short_description = "Author"

    def category_link(self, obj):
        if obj.category:
            return format_html('<a href="{}">{}</a>', 
                               f'/admin/archive/archivecategory/{obj.category.pk}/change/', 
                               obj.category.name)
        return "N/A"
    category_link.short_description = "Category"

    def parent_link(self, obj):
        if obj.parent:
            return format_html('<a href="{}">{}</a>', 
                               f'/admin/archive/archivestopic/{obj.parent.pk}/change/', 
                               obj.parent.title)
        return "N/A"
    parent_link.short_description = "Parent Forum"

    def has_poll_display(self, obj):
        try:
            return bool(obj.archive_poll)
        except models.ArchivePoll.DoesNotExist:
            return False
    has_poll_display.short_description = "Has Poll?"
    has_poll_display.boolean = True

    def get_queryset(self, request):
        return super().get_queryset(request).prefetch_related('archive_poll')


@admin.register(models.ArchiveCategory)
class ArchiveCategoryAdmin(admin.ModelAdmin):
    list_display = ('name', 'slug', 'is_hidden', 'topic_count')
    search_fields = ('name',)
    # inlines = [ArchiveTopicInline] # Can be very long if many topics.
    
    def topic_count(self, obj):
        return obj.archivetopic_set.count() # Counts all topics directly under this category
    topic_count.short_description = "Topics"

@admin.register(models.ArchiveForum)
class ArchiveForumAdmin(admin.ModelAdmin):
    list_display = ('name', 'total_users', 'total_messages', 'online_record', 'online_record_date')
    filter_horizontal = ('announcement_topics',)

@admin.register(models.ArchiveTopicReadStatus)
class ArchiveTopicReadStatusAdmin(admin.ModelAdmin):
    list_display = ('get_user_info', 'topic_link', 'last_read')
    search_fields = ('user_id', 'topic__title')
    readonly_fields = ('last_read', 'user')

    def get_user_info(self, obj):
        try:
            user = User.objects.using('default').get(pk=obj.user_id)
            url = reverse('admin:auth_user_change', args=[user.pk])
            return format_html('<a href="{}">{}</a>', url, user.username)
        except User.DoesNotExist:
            return f"User ID: {obj.user_id} (Not found)"
    get_user_info.short_description = "User"

    def topic_link(self, obj):
        if obj.topic:
            return format_html('<a href="{}">{}</a>', 
                               f'/admin/archive/archivestopic/{obj.topic.pk}/change/', 
                               obj.topic.title)
        return "N/A"
    topic_link.short_description = "Topic"

@admin.register(models.ArchiveSmileyCategory)
class ArchiveSmileyCategoryAdmin(admin.ModelAdmin):
    list_display = ('name', 'smiley_count')
    search_fields = ('name',)
    filter_horizontal = ('smileys',)

    def smiley_count(self, obj):
        return obj.smileys.count()
    smiley_count.short_description = "Smileys"

@admin.register(models.ArchivePoll)
class ArchivePollAdmin(admin.ModelAdmin):
    list_display = ('question', 'topic_link', 'created_at', 'days_to_vote', 'max_choices_per_user', 'can_change_vote', 'is_active_display')
    search_fields = ('question', 'topic__title')
    list_filter = ('created_at', 'days_to_vote', 'can_change_vote')
    inlines = [ArchivePollOptionInline]
    readonly_fields = ('created_at',)
    fieldsets = (
        (None, {
            'fields': ('topic', 'question')
        }),
        ('Voting Settings', {
            'fields': ('days_to_vote', 'max_choices_per_user', 'can_change_vote')
        }),
    )

    def topic_link(self, obj):
        if obj.topic:
            return format_html('<a href="{}">{}</a>', 
                               f'/admin/archive/archivestopic/{obj.topic.pk}/change/', 
                               obj.topic.title)
        return "N/A"
    topic_link.short_description = "Topic"

    def is_active_display(self, obj):
        return obj.is_active
    is_active_display.short_description = "Is Active?"
    is_active_display.boolean = True

@admin.register(models.ArchivePollOption)
class ArchivePollOptionAdmin(admin.ModelAdmin):
    list_display = ('text', 'poll_link', 'vote_count_display')
    search_fields = ('text', 'poll__question')
    readonly_fields = ('vote_count_display',)
    inlines = [ArchivePollOptionVotersInline]

    def poll_link(self, obj):
        if obj.poll:
            return format_html('<a href="{}">{}</a>',
                               f'/admin/archive/archivepoll/{obj.poll.pk}/change/',
                               obj.poll.question)
        return "N/A"
    poll_link.short_description = "Poll"

    def vote_count_display(self, obj):
        return obj.get_vote_count
    vote_count_display.short_description = "Votes"

...
def modern__index__processor(request, base_context):
    online_users_qs = FakeUser.objects.none()
...
def modern__archiveprofile_page__processor(request, base_context):
    ...
    recent_activity['last_month'] = ArchivePost.objects.filter(
        author=req_user,
        created_time__gte=now-timezone.timedelta(days=60),
        created_time__lte=now-timezone.timedelta(days=30)
    ).order_by('-created_time')

    all_posts = ArchivePost.objects.filter(author=req_user).order_by('-created_time')
    media_list = []
...
def modern__search_results__processor(request, base_context):
    ...
    for result in results:
        result.author_is_online = result.author.archiveprofile.last_login >= timezone.now() - timezone.timedelta(minutes=30)
    return {
        'header_size': 'small',
...
def modern__group_details__processor(request, base_context):
    ...
    for mod in mods:
        ...
        if group in mod.archiveprofile.groups.all():
            members.insert(0, mod) # Append at the top
    group_member_count = len(members)

    utf = ArchiveForum.objects.filter(name='UTF').first()
    total_count = ArchiveProfile.objects.all().count()  # Total number of profiles in the forum
    return {
        'header_size': 'small',
        'recently_active_users': get_recently_active_users(12), # For _stats_header.html
        'online': online,
...
def modern__groups__processor(request, base_context):
    ...
    for group in all_groups:
        ...
        members_list = list(ArchiveProfile.objects.filter(groups=group).select_related('top_group').prefetch_related('groups'))
...
def get_recently_active_users(limit=12): # To be used in _stats_header.html 
    ...
    users = FakeUser.objects.select_related('archiveprofile').filter(archiveprofile__isnull=False).order_by('-archiveprofile__last_login')[:limit]
    for user in users:
        user.archiveprofile.random_color = return_random_color(user.username)
    return users
...
def organize_online_users_by_groups(online_users_qs):
    ...
    online_users = online_users_qs.select_related('archiveprofile', 'archiveprofile__top_group').prefetch_related('archiveprofile__groups')
    ...
    for user in online_users:
        if user.archiveprofile:
            top_group = user.archiveprofile.get_top_group
